...
T(m+1,1:n+1)=(eva_heat(1,1:n+1)+B*(Train+273)+(LW(1,1:n+1))+((Dair_turbulent(1,1:n+1)*cp_air).*T_air/dely_air)+((DFlow(m,1:n+1)*rho_rain*cp_water).*(T_1(m,1:n+1)+273).*ety(m,1:n+1)/det))./(B+((Dair_turbulent(1,1:n+1)*cp_air)/dely_air)+((DFlow(m,1:n+1)*rho_rain*cp_water).*ety(m,1:n+1)/det));
